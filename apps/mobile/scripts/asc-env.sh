...
# An organization enrollment. The App Store Connect API does not
...
if [ ! -r "$ASC_ENV" ]; then
  ...
  echo "It must export ASC_KEY_ID, ASC_ISSUER_ID, ASC_KEY_PATH, APPLE_TEAM_ID," >&2
  echo "EAS_PROJECT_ID, SENTRY_ORG and SENTRY_PROJECT." >&2
  return 1
fi
...
for var in ASC_KEY_ID ASC_ISSUER_ID ASC_KEY_PATH APPLE_TEAM_ID EAS_PROJECT_ID SENTRY_ORG SENTRY_PROJECT; do
  if [ -z "${!var:-}" ]; then
